...
blockfrost_apikey = {"project_id" : "mainnetLFS6FXB9WbFOGjr3ut1zphm6Hwk78mKt"}
...
while complete_flag == 0:
    ...
    response_API = requests.get(blockfrost_address_base + address, headers=blockfrost_apikey)
    blockfrost_address_data = response_API.json()
    stake_address = blockfrost_address_data['stake_address']
    resposne_API2 = requests.get(blockfrost_stake_base + stake_address + '/addresses/assets', headers=blockfrost_apikey)
    ...
    while multiple_pages == 1:
        ...
        for asset in blockfrost_stake_data:
            if asset == 'lovelace':
                continue
            response_API3 = requests.get(blockfrost_asset_base + str(asset['unit']), headers=blockfrost_apikey)
            ...
            try:
                ...
                continue
        resposne_API_multipage = requests.get(blockfrost_stake_base + stake_address + '/addresses/assets?page=' + str(page), headers=blockfrost_apikey)
